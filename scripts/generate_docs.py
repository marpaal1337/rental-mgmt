...
def generate() -> str:
    ...
    a("| API | FastAPI bajo prefijo `/api` |")
    a("| Base de datos | SQLite (WAL + foreign_keys) con SQLModel + Alembic |")
    a("| Autenticación | API Key estática (`X-API-Key`) |")
    a("| Frontend | Vite + React 19 + TypeScript estricto + Ant Design + TanStack Query |")
    a("| Empaquetado | PyInstaller (onedir) + InnoSetup |")
    if pyproject["deps"]:
    ...
    a("```")
    a("")
    a("## 8. Empaquetado Windows")
    a("")
    a("Vía única: **PyInstaller (onedir autocontenido) + InnoSetup**. El equipo")
    a("destino no necesita Python.")
    a("")
    a("```bash")
    a("python scripts/build_windows_installer.py             # .exe + .zip")
    a("python scripts/build_windows_installer.py --innosetup # Solo .exe")
    a("python scripts/build_windows_installer.py --zip       # Solo .zip")
    a("```")
    a("")
    a("Flujo: icono → `npm run build` → `rental-mgmt.spec` → `build/innosetup.iss`.")
    a("")
